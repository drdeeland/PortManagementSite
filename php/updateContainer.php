<?php
$inData = getRequestInfo();

$dest_sid = $inData["dest_sid"];
$dest_tid = $inData["dest_tid"];
$source_sid = $inData["source_sid"];
$source_tid = $inData["source_tid"];
$storage_id = $inData["storage_id"];
$destIsShip = $inData["destIsShip"];
$sourceIsShip = $inData["sourceIsShip"];
$container_id = $inData["container_id"];

$conn = new mysqli("localhost","db","pass","portmanagement");

if ($conn->connect_error) {
	die("Connection failed: " . $conn->connect_error);
}
else {
    if ($destIsShip) {
        if ($sourceIsShip) {
            $dest_tid = null;
            $source_tid = null;

            $stmt = $conn->prepare("UPDATE Container_Location SET dest_sid=(?), dest_tid=(?), source_sid=(?), source_tid=(?), storage_id=(?) WHERE container_id=(?)");
            $stmt->bind_param("iiiiii", $dest_sid, $dest_tid, $source_sid, $source_tid, $storage_id, $container_id);
        }
        else {
            $dest_tid = null;
            $source_sid = null;

            $stmt = $conn->prepare("UPDATE Container_Location SET dest_sid=(?), dest_tid=(?), source_sid=(?), source_tid=(?), storage_id=(?) WHERE container_id=(?)");
            $stmt->bind_param("iiiiii", $dest_sid, $dest_tid, $source_sid, $source_tid, $storage_id, $container_id);
        }
    }
    else {
        if($sourceIsShip) {
            $dest_sid = null;
            $source_tid = null;

            $stmt = $conn->prepare("UPDATE Container_Location SET dest_sid=(?), dest_tid=(?), source_sid=(?), source_tid=(?), storage_id=(?) WHERE container_id=(?)");
            $stmt->bind_param("iiiiii", $dest_sid, $dest_tid, $source_sid, $source_tid, $storage_id, $container_id);
        }
        else {
            $dest_sid = null;
            $source_sid = null;

            $stmt = $conn->prepare("UPDATE Container_Location SET dest_sid=(?), dest_tid=(?), source_sid=(?), source_tid=(?), storage_id=(?) WHERE container_id=(?)");
            $stmt->bind_param("iiiiii", $dest_sid, $dest_tid, $source_sid, $source_tid, $storage_id, $container_id);
        }
    }

    $stmt->execute();

    $stmt->close();
    $conn->close();

    returnWithError("");
}

function getRequestInfo() {
	return json_decode(file_get_contents('php://input'), true);
}

// Function that sends an API response as a JSON object
function sendResultInfoAsJson( $obj ) {
	header('Content-type: application/json');
	echo $obj;
}

// Function that returns with an error as a JSON object (Used just to send back a response)
function returnWithError( $err ) {
	$retValue = '{"id":0,"error":"' . $err . '"}';
	sendResultInfoAsJson( $retValue );
}

function returnWithInfo( $info ) {
    $retValue = '{"results":[' . $info . '],"error":""}';
    sendResultInfoAsJson( $retValue );
}
?>
